...
### 规范:
- 代码规范, 风格, 提前约定.
- 版本管理, 推荐使用[[Git]], ~~SVN 使用分支注意! 上线后相关重大, 平时保持良好习惯~~.
- 注释可文档化, 方便新人了解及旧人交接. 考虑 doxygen
...
- 数据缓存服务器 `DBServer` 过于复杂, 逻辑添加过于繁冗.  
  ...
  由于 `DBServer` 的原因, 为满足这些功能, 进行了对部分表进行了预加载.  
- 注意数据的划分,  
  方便逻辑异步执行, 便于多线程化
- 模块耦合度注意,   
  降低关联, 减少历史负担, 减少编译时间(C/C++ 头文件依赖)...
- 提前考虑用户ID设计:  
  为运维需求添加了相同 `GlobalServer` 下唯一的id, 其功能从代码逻辑来看完全可以替代角色id.
